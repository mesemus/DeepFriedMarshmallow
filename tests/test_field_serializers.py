...
def field_serializer_factory(field_obj, context):
    if isinstance(field_obj, AttributeInt):
        return AttributeFieldSerializer(context)
    return None
...
def test_attribute_field_serializer_factory(monkeypatch):
    monkeypatch.setattr(_registry, "field_serializer_factories", [field_serializer_factory])
...
def test_jitted_schema(monkeypatch):
    ...
    result = serialize_method(TestData(a_attribute=3, a=4))
    expected = {"a_attribute": 3, "a": 4}
