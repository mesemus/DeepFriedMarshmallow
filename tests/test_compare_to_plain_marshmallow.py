...
from marshmallow.fields import (
    ...
    Date, DateTime, NaiveDateTime, AwareDateTime, Time, TimeDelta,
    UUID, Url, Email, IP, IPv4, IPv6,
    Constant, Raw, List, Dict, Tuple, Nested, IPInterface, IPv4Interface, IPv6Interface,
    Enum as EnumField, Function, Method,
)
...
@pytest.mark.parametrize("field, field_value", [
    ...
    pytest.param(Integer(validate=[validate.Range(min=0), validate.Range(max=100)]), -1,  id="MultiValidator-fails-first"),
    pytest.param(Integer(validate=[validate.Range(min=0), validate.Range(max=100)]), 101, id="MultiValidator-fails-second"),
    pytest.param(
        String(validate=[validate.Length(min=2), validate.Regexp(r"^\d+$")]),
        "42",   id="MultiValidator-str-valid",
    ),
    pytest.param(
        String(validate=[validate.Length(min=2), validate.Regexp(r"^\d+$")]),
        "4",    id="MultiValidator-str-fails-length",
    ),
    pytest.param(
        String(validate=[validate.Length(min=2), validate.Regexp(r"^\d+$")]),
        "ab",   id="MultiValidator-str-fails-regexp",
    ),
    ...
    pytest.param(Method(deserialize="validate_positive", required=True), 10,   id="Method-validate-positive-required-valid"),
    pytest.param(Method(deserialize="validate_positive", required=True), None, id="Method-validate-positive-required-none"),
    ...
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"name": "Alice", "age": 30},                              id="Nested-person-valid"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"name": "Alice", "age": 30, "score": 9.5},               id="Nested-person-valid-all-fields"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"name": "Alice"},                                          id="Nested-person-optional-absent"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"age": 30},                                                id="Nested-person-missing-required"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"name": "Alice", "age": "not-a-number"},                  id="Nested-person-invalid-field-type"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {},                                                          id="Nested-person-empty"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 "not-a-dict",                                               id="Nested-person-invalid-scalar"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 None,                                                        id="Nested-person-none"),
    pytest.param((Nested(_PersonSchema, required=True), Nested(_PersonSchemaJit, required=True)),
                 {"name": "Alice"},                                           id="Nested-person-required-valid"),
    pytest.param((Nested(_PersonSchema, required=True), Nested(_PersonSchemaJit, required=True)),
                 None,                                                        id="Nested-person-required-none"),
    pytest.param((Nested(_PersonSchema, allow_none=True), Nested(_PersonSchemaJit, allow_none=True)),
                 None,                                                        id="Nested-person-allow-none"),

    # --- Nested many=True (_PersonSchema) ---
    pytest.param((Nested(_PersonSchema, many=True), Nested(_PersonSchemaJit, many=True)),
                 [{"name": "Alice", "age": 30}, {"name": "Bob"}],           id="Nested-person-many-valid"),
    pytest.param((Nested(_PersonSchema, many=True), Nested(_PersonSchemaJit, many=True)),
                 [{"age": 30}],                                               id="Nested-person-many-missing-required"),
    pytest.param((Nested(_PersonSchema, many=True), Nested(_PersonSchemaJit, many=True)),
                 [],                                                           id="Nested-person-many-empty"),
    pytest.param((Nested(_PersonSchema, many=True), Nested(_PersonSchemaJit, many=True)),
                 None,                                                         id="Nested-person-many-none"),
    pytest.param((Nested(_PersonSchema, many=True), Nested(_PersonSchemaJit, many=True)),
                 "not-a-list",                                                 id="Nested-person-many-invalid-scalar"),

    # --- Nested (_AddressSchema) ---
    pytest.param((Nested(_AddressSchema), Nested(_AddressSchemaJit)),
                 {"street": "Main St", "city": "Springfield"},               id="Nested-address-valid"),
    pytest.param((Nested(_AddressSchema), Nested(_AddressSchemaJit)),
                 {"street": "Main St", "city": "Springfield", "zip_code": "12345"}, id="Nested-address-valid-all-fields"),
    pytest.param((Nested(_AddressSchema), Nested(_AddressSchemaJit)),
                 {"street": "Main St"},                                       id="Nested-address-missing-required"),
    pytest.param((Nested(_AddressSchema), Nested(_AddressSchemaJit)),
                 {},                                                           id="Nested-address-empty"),
    pytest.param((Nested(_AddressSchema), Nested(_AddressSchemaJit)),
                 None,                                                         id="Nested-address-none"),
    pytest.param((Nested(_AddressSchema, required=True), Nested(_AddressSchemaJit, required=True)),
                 {"street": "Main St", "city": "Springfield"},               id="Nested-address-required-valid"),
    pytest.param((Nested(_AddressSchema, required=True), Nested(_AddressSchemaJit, required=True)),
                 None,                                                         id="Nested-address-required-none"),
    pytest.param((Nested(_AddressSchema, allow_none=True), Nested(_AddressSchemaJit, allow_none=True)),
                 None,                                                         id="Nested-address-allow-none"),

    # --- Nested with only / exclude ---
    pytest.param((Nested(_PersonSchema, only=("name",)), Nested(_PersonSchemaJit, only=("name",))),
                 {"name": "Alice"},                                            id="Nested-person-only-valid"),
    pytest.param((Nested(_PersonSchema, only=("name",)), Nested(_PersonSchemaJit, only=("name",))),
                 {},                                                            id="Nested-person-only-missing-required"),
    pytest.param((Nested(_PersonSchema, exclude=("score",)), Nested(_PersonSchemaJit, exclude=("score",))),
                 {"name": "Alice", "age": 30},                                id="Nested-person-exclude-valid"),
    pytest.param((Nested(_PersonSchema, exclude=("score",)), Nested(_PersonSchemaJit, exclude=("score",))),
                 {"name": "Alice", "age": "bad"},                             id="Nested-person-exclude-invalid-field"),

    # --- Nested inside Nested (_CompanySchema → _AddressSchema) ---
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 {"name": "ACME", "address": {"street": "Main St", "city": "Springfield"}},
                 id="Nested-nested-valid"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 {"name": "ACME", "address": {"street": "Main St", "city": "Springfield", "zip_code": "12345"}, "employees": ["Alice", "Bob"]},
                 id="Nested-nested-valid-all-fields"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 {"name": "ACME"},
                 id="Nested-nested-inner-absent"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
    ...
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 {},
                 id="Nested-nested-outer-missing-required"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 None,
                 id="Nested-nested-none"),
    pytest.param((Nested(_CompanySchema, required=True), Nested(_CompanySchemaJit, required=True)),
                 {"name": "ACME", "address": {"street": "Main St", "city": "Springfield"}},
                 id="Nested-nested-required-valid"),
    pytest.param((Nested(_CompanySchema, required=True), Nested(_CompanySchemaJit, required=True)),
                 None,
                 id="Nested-nested-required-none"),
    pytest.param((Nested(_CompanySchema, allow_none=True), Nested(_CompanySchemaJit, allow_none=True)),
                 None,
                 id="Nested-nested-allow-none"),

    # --- List of Nested (_PersonSchema) ---
    pytest.param((List(Nested(_PersonSchema)), List(Nested(_PersonSchemaJit))),
    ...
    pytest.param((List(Nested(_PersonSchema)), List(Nested(_PersonSchemaJit))),
                 None,
                 id="List-nested-person-none"),
    pytest.param((List(Nested(_PersonSchema), required=True), List(Nested(_PersonSchemaJit), required=True)),
                 [{"name": "Alice"}],
                 id="List-nested-person-required-valid"),
    pytest.param((List(Nested(_PersonSchema), required=True), List(Nested(_PersonSchemaJit), required=True)),
                 None,
                 id="List-nested-person-required-none"),
    pytest.param((List(Nested(_PersonSchema), allow_none=True), List(Nested(_PersonSchemaJit), allow_none=True)),
                 None,
                 id="List-nested-person-allow-none"),

    # --- Dict[String, Nested(_PersonSchema)] ---
    pytest.param((Dict(keys=String(), values=Nested(_PersonSchema)), Dict(keys=String(), values=Nested(_PersonSchemaJit))),
    ...
    pytest.param(
        ...
        {"a": 1, "b": 2},   id="Dict-str-int-inner-validate-valid",
    ),
    pytest.param(
        Dict(keys=String(), values=Integer(validate=validate.Range(min=0))),
        {"a": 1, "b": -1},  id="Dict-str-int-inner-validate-invalid",
    ),
    pytest.param(
        Dict(keys=String(validate=validate.Length(min=1)), values=String()),
        {"ab": "x"},        id="Dict-key-validate-valid",
    ),
    pytest.param(
        Dict(keys=String(validate=validate.Length(min=1)), values=String()),
        {"": "x"},          id="Dict-key-validate-invalid",
    ),
])
...
_SERIALIZE_CASES = [
    ...
    # --- AwareDateTime ---
    pytest.param(AwareDateTime(), datetime.datetime(2023, 1, 15, 10, 30, 0, tzinfo=datetime.timezone.utc), id="AwareDateTime-valid"),
    pytest.param(AwareDateTime(), None,                                                                     id="AwareDateTime-none"),
    ...
    # --- Nested (_PersonSchema) ---
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"name": "Alice", "age": 30, "score": 9.5},               id="Nested-person-valid-all"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {"name": "Alice"},                                          id="Nested-person-partial"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 {},                                                          id="Nested-person-empty"),
    pytest.param((Nested(_PersonSchema), Nested(_PersonSchemaJit)),
                 None,                                                        id="Nested-person-none"),
    pytest.param((Nested(_PersonSchema, allow_none=True), Nested(_PersonSchemaJit, allow_none=True)),
                 None,                                                        id="Nested-person-allow-none"),
    pytest.param((Nested(_PersonSchema, only=("name",)), Nested(_PersonSchemaJit, only=("name",))),
                 {"name": "Alice", "age": 30},                               id="Nested-person-only"),
    pytest.param((Nested(_PersonSchema, exclude=("score",)), Nested(_PersonSchemaJit, exclude=("score",))),
                 {"name": "Alice", "age": 30, "score": 9.5},                id="Nested-person-exclude"),
    ...
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 ...
                 id="Nested-nested-valid"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 {"name": "ACME", "address": None},                          id="Nested-nested-inner-none"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 {"name": "ACME"},                                            id="Nested-nested-inner-absent"),
    pytest.param((Nested(_CompanySchema), Nested(_CompanySchemaJit)),
                 None,                                                         id="Nested-nested-none"),

    # --- List of Nested ---
    pytest.param((List(Nested(_PersonSchema)), List(Nested(_PersonSchemaJit))),
                 [{"name": "Alice", "age": 30}, {"name": "Bob"}],           id="List-nested-person-valid"),
    pytest.param((List(Nested(_PersonSchema)), List(Nested(_PersonSchemaJit))),
                 [],                                                           id="List-nested-person-empty"),
    pytest.param((List(Nested(_PersonSchema)), List(Nested(_PersonSchemaJit))),
                 None,                                                         id="List-nested-person-none"),

    # --- Dict of Nested ---
    pytest.param((Dict(keys=String(), values=Nested(_PersonSchema)), Dict(keys=String(), values=Nested(_PersonSchemaJit))),
                 {"alice": {"name": "Alice", "age": 30}},                   id="Dict-nested-person-valid"),
    pytest.param((Dict(keys=String(), values=Nested(_PersonSchema)), Dict(keys=String(), values=Nested(_PersonSchemaJit))),
                 {},                                                           id="Dict-nested-person-empty"),
    pytest.param((Dict(keys=String(), values=Nested(_PersonSchema)), Dict(keys=String(), values=Nested(_PersonSchemaJit))),
                 None,                                                         id="Dict-nested-person-none"),
]
